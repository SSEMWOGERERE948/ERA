...
const kil = () => {
  const [kilData, setKilData] = useState([]);
  ...
  const fetchKilData = async () => {
    try {
      const surchargeRef = collection(db, 'surchargeReports');
      const q = query(surchargeRef, where('selectedDistributor', '==', 'KIL'));
      ...
      console.error('Error fetching kil data:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>KIL Data</Text>
      {selectedLocation && (
