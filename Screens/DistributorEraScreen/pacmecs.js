...
const pacmecs = () => {
  const [pacmecsData, setpacmecsData] = useState([]);
  ...
  const fetchpacmecsData = async () => {
    try {
      const surchargeRef = collection(db, 'surchargeReports');
      const q = query(surchargeRef, where('selectedDistributor', '==', 'PACMECS'));
      ...
      console.error('Error fetching pacmecs data:', error);
    }
  };
