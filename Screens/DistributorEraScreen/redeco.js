...
const redeco = () => {
  const [redecoData, setredecoData] = useState([]);
  ...
  const fetchredecoData = async () => {
    try {
      const surchargeRef = collection(db, 'surchargeReports');
      const q = query(surchargeRef, where('selectedDistributor', '==', 'REDECO'));
      ...
      console.error('Error fetching redeco data:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>redeco Data</Text>
      {selectedLocation && (
