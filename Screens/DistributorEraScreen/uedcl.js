...
const uedcl = () => {
  const [uedclData, setuedclData] = useState([]);
  ...
  const fetchuedclData = async () => {
    try {
      const surchargeRef = collection(db, 'surchargeReports');
      const q = query(surchargeRef, where('selectedDistributor', '==', 'UEDCL'));
      ...
      console.error('Error fetching uedcl data:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>UEDCL Data</Text>
      {selectedLocation && (
