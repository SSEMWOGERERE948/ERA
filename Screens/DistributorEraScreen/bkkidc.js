...
const bkkidc = () => {
  const [bkkidcData, setbkkidcData] = useState([]);
  ...
  const fetchbkkidcData = async () => {
    try {
      const surchargeRef = collection(db, 'surchargeReports');
      const q = query(surchargeRef, where('selectedDistributor', '==', 'BKKIDC'));
      ...
      console.error('Error fetching bkkidc data:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>bkkidc Data</Text>
      {selectedLocation && (
